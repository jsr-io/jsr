...
// Polyfills the subset of the Deno global needed by the frontend at runtime
// so the Fresh build output can run inside the Cloudflare Workers runtime.
//
// Only filesystem and env reads are shimmed — anything else genuinely
// requiring Deno (subprocess, openKv, ...) is not used at runtime by this
// app.
...
if (typeof g.Deno === "undefined") {
  ...
  g.Deno = {
    ...
    // Stat is best-effort: HEAD the asset and surface size if reported.
    // Fresh's `ProdBuildCache.readFile` calls Deno.stat + Deno.open together
    // — under normal Workers operation those paths are served from the
