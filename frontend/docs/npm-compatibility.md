...
Tarballs are served at the same path layout that `registry.npmjs.org` uses. For
example, the tarball for version `1.0.0` of `@jsr/luca__cases` is served at
`https://npm.jsr.io/@jsr/luca__cases/-/luca__cases-1.0.0.tgz`. This allows JSR
to be used as an upstream for npm private registries that reconstruct tarball
paths using the npm registry's conventions, such as JFrog Artifactory and Google
Artifact Registry.

The contents of a tarball may be re-generated by the JSR registry to fix
...
the `package.json` file in the tarball at the `_jsr_revision` field. The
revision of a tarball is not considered part of the package version, and does
not affect semver resolution.

Historically, tarballs were served from revision-specific URLs (of the form
`https://npm.jsr.io/~/11/@jsr/luca__cases/1.0.0.tgz`). These URLs continue to be
available forever, so package manager lock files that reference them keep
working.
