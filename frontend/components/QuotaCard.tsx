...
export function QuotaCard(
  ...
  return (
    <div class="border-1.5 border-jsr-gray-200 dark:border-jsr-gray-700 rounded-md px-4 py-5 flex flex-col justify-between dark:bg-jsr-gray-900">
      <div>
        <p class="font-semibold text-jsr-gray-900 dark:text-gray-100">{props.title}</p>
        <p class="text-jsr-gray-600 dark:text-gray-400 text-sm">{props.description}</p>
      </div>
...
function QuotaUsage(props: { limit: number; usage: number }) {
  ...
  return (
    <div class="mt-4 flex items-center gap-2">
      <div
        class={`overflow-hidden h-3 w-full rounded bg-jsr-yellow-50 dark:bg-jsr-gray-800 ring-1 ${ringColor}`}
      >
        <div
      ...
      <div class="text-xs text-jsr-gray-600 dark:text-gray-400">{props.usage}/{props.limit}</div>
    </div>
  );
}
