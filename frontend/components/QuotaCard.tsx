...
export function QuotaCard(
  ...
  return (
    <div class="border-1.5 border-jsr-gray-200 rounded-md px-4 py-5 flex flex-col justify-between">
      <div>
        <p class="font-semibold text-jsr-gray-900 dark:text-jsr-gray-100">{props.title}</p>
        <p class="text-jsr-gray-600 text-sm dark:text-jsr-gray-400">{props.description}</p>
      </div>
...
function QuotaUsage(props: { limit: number; usage: number }) {
  ...
  return (
    <div class="mt-4 flex items-center gap-2">
      ...
      <div class="text-xs text-jsr-gray-600 dark:text-jsr-gray-200">{props.usage}/{props.limit}</div>
    </div>
  );
}
