...
locals {
  ...
  api_envs = merge(local.otlp_envs, {
    ...
    # The DB requires a client certificate (ssl_mode =
    # TRUSTED_CLIENT_CERTIFICATE_REQUIRED, see db.tf), so both Cloud Run services
    # present it over the private VPC IP; the container gets the same cert via
    # worker secrets (see lb.tf). Plain env, like DATABASE_URL / S3_SECRET_KEY.
...
# The API container runs on Cloudflare, not GCP, so it can't mint GCP access
# tokens from the instance metadata server. This key is handed to the
# container via the LB worker's METADATA_STRATEGY binding (see lb.tf), and the
# API uses it to sign a JWT and exchange it for an access token (see gcp.rs).
