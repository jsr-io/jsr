...
function apiEnvVars(env: WorkerEnv): Record<string, string> {
  return {
    ...
    DB_CLIENT_CERT: env.DB_CLIENT_CERT ?? "",
    DB_CLIENT_KEY: env.DB_CLIENT_KEY ?? "",
    DB_ROOT_CERT: env.DB_ROOT_CERT ?? "",
...
// The Rust API server, running as a Cloudflare Container. Only the request
// serving path (`--api`) runs here; background tasks (`--tasks`) stay on
// Cloud Run since a sleeping container can't run scheduled work.
