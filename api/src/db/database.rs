...
/// Client-certificate TLS material for connecting to the database. Used when
/// the API runs off-GCP (the Cloudflare Container) and reaches Cloud SQL over
/// the public IP, where a client certificate is the access boundary.
pub struct DbTls {
...
impl Database {
  ...
  pub async fn get_user(&self, id: Uuid) -> Result<Option<User>> {
    query_concat_as!(
      User,
      "SELECT ", USER_SELECT_FULL, " FROM users WHERE id = $1";
      id
    )
  ...
  pub async fn list_users(
    ...
    let users: Vec<User> = sqlx::query_as(&format!(
      ...
      crate::db::sql_fragments::USER_SELECT_FULL_RT,
    ))
    .bind(&search)
    .bind(maybe_id)
    .bind(start)
...
gitlab_id: r.user_gitlab_id,
  ...
  pub async fn get_scope_member(
    ...
    query_concat_as!(
      ScopeMember,
      "SELECT ", SCOPE_MEMBER_SELECT, " FROM scope_members WHERE scope = $1 AND user_id = $2";
      scope as _,
      user_id
    )
      .fetch_optional(&self.pool)
  ...
  pub async fn list_scope_members(
    ...
    query_concat!(
      "SELECT ", SCOPE_MEMBER_SELECT_JOINED, ",
  ...
  pub async fn get_scope_invites_by_user(
    ...
    query_concat!(
      "SELECT ", SCOPE_INVITE_SELECT_JOINED, "
      FROM scope_invites
...
"#,
      ...
      crate::db::sql_fragments::USER_SELECT_FULL_JOINED_RT,
    ))
    .bind(&search)
    .bind(start)
    .bind(limit)
