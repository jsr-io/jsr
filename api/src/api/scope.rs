...
async fn create_handler(mut req: Request<Body>) -> ApiResult<ApiScope> {
  ...
  let scope = db
    .create_scope(None, &scope, user.id)
    .await
...
pub async fn delete_handler(req: Request<Body>) -> ApiResult<Response<Body>> {
  ...
  let deleted = db.delete_scope(sudo.then_some(&user.id), &scope).await?;
  if !deleted {
...
async fn invite_member_handler(
  ...
  let scope_invite = db
    .add_scope_invite(
      sudo.then_some(&current_user.id),
      NewScopeInvite {
        scope: &scope,
...
pub async fn delete_invite_handler(
  ...
  db.delete_scope_invite(sudo.then_some(&user.id), &user_id, &scope)
    .await?;
...
pub mod tests {
  ...
  async fn scope_members_add_as_admin() {
    ...
    t.db()
      .create_scope(None, &scope_name, t.user1.user.id)
      .await
      .unwrap();
  ...
  async fn scope_transfer() {
    ...
    for i in 0..3 {
      ...
      t.db()
        .create_scope(None, &scope_name, t.user2.user.id)
        .await
        .unwrap();
    }
